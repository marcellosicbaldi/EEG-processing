%% REST TASK REST
% Long recording lasting 15 minutes. First 5 min: relax phase R1. Successive 
% 5 min: task phase T (solving arithmetic operations and providing a solution 
% choosing one out of three possible choices). Final 5 min: relax phase R2.
% 
% I apply ICA to all the recording, and after preprocessing I want to analyze 
% the processed data. The analysis involves the alpha rhythm (8-13/14 Hz), which 
% we know is dominant during the relaxig phase. I want to see how it changes during 
% T and R2. During the relaxed condition alpha is dominant in the posterior region 
% of the scalp. It has the function of inhibiting processing of external stimuli, 
% in particular visual (visual cortex is located posteriorly) --> helps the subject 
% to isolate from the external enviromìnment (INTERNAL ATTENTION mechanism).
% 
% Frequency domain analysis: I want to compute in each phase the PSD (using 
% pwelch) of the 13 signals. Then I group the 13 electrodes in 3 macro regions 
% of the scalp
%% 
...
%% 
% And I average the PSD of the channels in each macro region, to obtain PSD_frontal, 
% PSD_tempcentr, PSD_parietocc for each of the three phases!! Total of 9 values. 
% To visually inspect the results I plot in a single subplot PSD_frontal for the 
% three different phases, and same for tempcentr and parietocc in other two subplots. 
% Expected result: peak around 10 Hz in R1 and R2 (more prominent in the posterior 
% regions). The peak should be greatly attenuated during the task.
% 
% Finally I can compute the alpha power in each region and for each phase (9 
% values, integral of PSD in range 8-14 Hz using _trapz_). I can then plot those 
% values.
...
PSD = PSD';
%% 
% I now use EEGLAB to estimate the ICs and import both demixing matrix W and 
% mixing matrix A (S = WX)
...
% 
% IC4 spectrum mainly limited to very low frequencies + large artifact as IC1 
% around 300 s
% 
% IC8 strange spectrum with high frequencies content and large artifact around 
% 300 s
% 
% Also IC9, IC10 and IC11 seem to contain some artifact activity, but I have 
% maintained them as they also contain alpha power
...
PSD_R2 = PSD_R2';
%% 
% By visually comparing the three phases, we can observe how during the task 
% the occipital alpha power is much lower wrt the relax phases. In which relax 
% phase is the alpha power higher? I can compute the mean peak of the power in 
% the posterior electrodes (from PO7 to O2) to get some first insights 

mean_PSDpeak_R1 = mean(max(PSD_R1(PO7:O2, 8/0.1:14/0.1), [], 2))
mean_PSDpeak_R2 = mean(max(PSD_R2(PO7:O2, 8/0.1:14/0.1), [], 2))
%% 
% We can see that the mean peak is higher in the R2 phase. Possible explanation: 
% during R1 the subject knows that he will eventually be presented with a stimulus 
% --> not completely relaxed as in R2
...
%% 
% Frontal: low alpha activity during R1 and R2, but still higher wrt to the 
% task in all three regions
%% 
% Temporal-Central
...
%% 
% Temporal-central --> higher alpha band activity than frontal but still much 
% lower than posterior
%% 
% Parietal-Occipital
...
% 
% We can observe that for all three regions the alpha activity is greatly attenuated 
% during the task
