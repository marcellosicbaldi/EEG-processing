...
% The goal of this short project is to process, analyze and interpret the EEG 
% of a single healthy subject who performed a 3-stimulus oddball task. 		
% 
% The classical 2-stimulus oddball task consists in presenting repeatedly two 
% different stimuli in a random sequence: one stimulus, called standard, occurs 
% frequently, while the other stimulus occurs infrequently and is called target 
% since the subject is instructed to respond in some way to this rare stimulus. 
% The 3-stimulus oddball task is a modification of the 2-stimulus task where a 
% third stimulus is added, occurring infrequently as the target stimulus, and 
% called distractor. In this case too, the subject has to respond only to the 
% target stimulus. In the specific 3-stimulus auditory oddball task, the standard 
% stimulus was a 440 Hz sinusoidal tone (70% of trials), the target stimulus was 
% a 660 Hz sinusoidal tone (15% of trials) and the distractor (15% of trials) 
% was a naturalistic sound. Each stimulus lasted 200 ms. The subject was instructed 
% to mentally count the target stimuli and ignoring the standard and distractor 
% stimuli
% Data loading
...
% 
% I half the sampling rate --> My new data contains half of the samples --> 
% I need also to half the time samples of the markers 
% 
% Matlab function _*resample*_: automatically applies an antialiasing (LP) filter. 
% Why? The sampling frequency must always be at least twice the maximum frequency 
% of interest (Shannon theorem) --> I have to attenuate the frequencies components 
% above half the new sampl freq. (Above 250 Hz in this case).
...
t = X_samples/fs;
% Detrend
% Done to remove possible slow linear drifts, due to noise originating at the 
% electrode-gel interface.
% 
% It removes from the data the line that best fits the data in the least square 
% sense.
...
grid
% Epoching 
% It consists in extracting a segment (or epoch) around the time position of 
% each marker of interest, with each epoch containing a pre-event period and a 
% post-event period; all epochs must be of the same length (usually lasting a 
% few tens of milliseconds). After epoching, the extracted epochs can be stored 
% in a 3-D matrix i.e. _electrodes x samples x epochs_.
% 
% I am going to extract epochs starting 200 ms before each marker and ending 
% 800 after the marker, for a total duration of 1s.
...
%% 
% Concatenating the epochs along the time dimension, returning back to a 2D 
% matrix. Necessary since ICA conceptually works on that kind of representation.
...
% Correlation method: bad channels are identified as those having low correlation 
% (below a given threshold) with the other channels.
...
%% 
% IC1 and IC2 blink artifact ---- IC3 artifact on all electrodes (or on the 
% reference electrode)
% 
...
% 
% IC17 and IC19 clear EMG artifact due to activation of small muscle fibers 
% (highly localized)
% 
...
% ERP analysis
% Baseline correction
% Consists in removing from each epoch the mean value of the signal over a baseline 
% pre-event period. Useful to counteract baseline differences between data epochs 
% (e.g., those arising from low-frequency drifts).
...
% Averaging technique
% Consists in computing, in each time instant inside the epoch, the average 
% value across the epochs. Done to increase the signal to noise ratio. (Signal: 
% evoked response. Noise: background EEG activity)
...
%% 
% The ERP waveform associated to the target stimulus displays P300, which is 
% maximal over the parietal and central electrodes, while the P300 is not present 
% in the ERP waveform associated to the standard stimulus. The P3 peak has been 
% associated to voluntary attentional components, recognition of stimulus relevance 
% for the task, and memory updating. 	The P3 evoked by this two-stimulus oddball 
% task is often referred to as P3b, in order to distinguish it from a variant, 
% called P3a.	
% 
% The latter appears when a third ‘novel’ (still rare) stimulus is inserted 
% into the oddball task, and the subject is not required to respond to this stimulus 
% (also called ‘distractor’). ‘Novelty’ refers to the context of the other two 
% stimuli. Such novel stimuli elicit a large positivity (P3a) with a latency that 
% is earlier than that of the target-evoked P3b, and a scalp distribution that 
% is more oriented towards the front of the scalp. P3a has been linked to involuntary 
...
%% 
% TARGET STIMULUS: ERS in the theta band at 100-400 ms in the frontal end especially 
% in the central regions, and ERD in the alpha and theta band at 400-700 ms especially 
% in frontal and posterior regions
% 
